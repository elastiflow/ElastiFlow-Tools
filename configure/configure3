...
check_service_exists() {
  if ! systemctl status flowcoll.service &>/dev/null; then
    print_message "flowcoll.service does not exist. Exiting." "$RED"
    exit 1
  fi
}
...
check_service_health() {
  ...
  if ! sudo systemctl is-active --quiet flowcoll.service; then
    ...
    if sudo journalctl -u flowcoll.service | grep -q "license error"; then
      print_message "License error found in logs. Rerunning trial configuration." "$RED"
      configure_trial
      reload_and_restart_flowcoll
    else
...
configure_trial() {
  ...
  if check_service_health configure_trial; then
    ...
    restore_latest_backup
    print_message "Failed to enable fully featured trial. Changes reverted." "$RED"
  fi
}
...
configure_maxmind() {
  ...
  if sudo wget -O ./Geolite2-ASN.tar.gz "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-ASN&license_key=$maxmind_license_key&suffix=tar.gz"; then
    ...
    print_message "Failed to download MaxMind ASN database. Rerunning the configuration routine." "$RED"
    configure_maxmind
    return 1
  fi
  ...
  if sudo wget -O ./Geolite2-City.tar.gz "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-City&license_key=$maxmind_license_key&suffix=tar.gz"; then
    ...
    print_message "Failed to download MaxMind GeoIP City database. Rerunning the configuration routine." "$RED"
    configure_maxmind
    return 1
  fi
...
capture_packets() {
  ...
  # Prompt for capture duration and port filter
  while true; do
  ...
  while true; do
    read -p "Enter the port to filter for (or 'q' to quit): " port
    if [[ $port == "q" ]]; then
      echo "Exiting."
      return
    elif [[ ! $port =~ ^[0-9]+$ ]]; then
  ...
  # Capture packets
  timestamp=$(date +%Y%m%d%H%M%S)
  sudo tcpdump -i $interface port $port -w "capture_$timestamp.pcap" -G $duration -W 1
  ...
  case $choice in
    ...
    3)
      while true; do
        ...
        echo "9. Back"
        read -p "Enter your choice (1-9): " utility_choice
        case $utility_choice in
          ...
          9)
            ...
            ;;
          *)
            print_message "Invalid choice. Please enter a number between 1 and 9." "$RED"
            ;;
        esac
      done
      ;;
      ...
      while true; do
        ...
        case $game_choice in
          ...
          2)
            ...
            ;;
          *)
            print_message "Invalid choice. Please enter a number between 1 and 2." "$RED"
            ;;
        esac
      done
      ;;
