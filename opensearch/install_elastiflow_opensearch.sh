...
flow_dashboards_version="2.14.x"
...
configure_snapshot_repo() {
  # Add snapshot path configuration to opensearch.yml
  echo -e "\n# Path to snapshots:\npath.repo: /etc/opensearch/snapshots" | tee -a /etc/opensearch/opensearch.yml

  # Create snapshots directory and set ownership
  mkdir -p /etc/opensearch/snapshots
  chown -R opensearch:opensearch /etc/opensearch/snapshots

  # Restart Opensearch service
  systemctl restart opensearch.service
  if ! systemctl is-active --quiet opensearch.service; then
    echo "Failed to restart Opensearch service. Exiting."
    exit 1
  fi

  # Wait for Opensearch to be fully up and running
  sleep 10

  # Create the snapshot repository via Opensearch API

  curl -s -u "$opensearch_username:$opensearch_password2" -X PUT "http://localhost:9200/_snapshot/my_snapshot" \
    -H "Content-Type: application/json" \
    -d '{
          "type": "fs",
          "settings": {
            "location": "/etc/opensearch/snapshots",
            "compress": true
          }
        }' || {
...
check_for_script_updates() {
  ...
  local new_script_url="https://raw.githubusercontent.com/elastiflow/ElastiFlow-Tools/main/openearch/install.sh"
...
EOF
  ...
  tee /etc/systemd/system/opensearch.service.d/opensearch.conf > /dev/null
...
install_dashboards() {
  ...
  response=$(curl --connect-timeout 10 -X POST -u $opensearch_username:$opensearch_password2 "localhost:5601/api/saved_objects/_import?overwrite=true" -H "osd-xsrf: true" --form file=@/etc/elastiflow_for_opensearch/dashboards/flow/dashboards-$flow_dashboards_version-flow-$flow_dashboards_codex_ecs.ndjson -H 'osd-xsrf: true')
...
configure_opensearch() {
  ...
  find_and_replace "/etc/opensearch/opensearch.yml" "${config_strings[@]}"
}
...
check_all_services() {
  ...
  for SERVICE_NAME in "${SERVICES[@]}"; do
      print_message "Restarting $SERVICE_NAME..."
      systemctl restart $SERVICE_NAME
      sleep_message "Waiting for service $SERVICE_NAME to stabilize." 5
  done
  for SERVICE_NAME in "${SERVICES[@]}"; do
...
set_kibana_homepage() {
  ...
  local find_response=$(curl -s -u "$opensearch_username:$opensearch_password2" -X GET "$kibana_url/api/saved_objects/_find?type=dashboard&search_fields=title&search=$encoded_title" -H 'osd-xsrf: true')
  ...
  if [ -z "$dashboard_id" ]; then
    ...
    local update_response=$(curl -s -o /dev/null -w "%{http_code}" -u "$opensearch_username:$opensearch_password2" \
...
main() {
  ...
  print_message "Go to http://$ip_address:5601 (admin / "$opensearch_password2")\n\n\e[0m"
  print_message "**************************" "$GREEN"
  }
