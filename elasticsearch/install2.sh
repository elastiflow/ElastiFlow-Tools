...
comment_and_replace_line() {
  ...
  if grep -Eq "^[#]*$FIND_ESCAPED" "$FILE"; then
    original_line=$(grep "^[#]*$FIND_ESCAPED" "$FILE")
    sed -i.bak "/^[#]*$FIND_ESCAPED/c\\$REPLACE" "$FILE"
    print_message "Replaced existing line '$original_line' with '$REPLACE'." "$GREEN"
  else
...
find_and_replace() {
  ...
  for ((i = 0; i < ${#PAIRS[@]}; i+=2)); do
    ...
    comment_and_replace_line "$FILE" "$FIND" "$REPLACE"
  done
...
elastic_install_log=$(apt-get -qq update && apt-get -qq install elasticsearch=$elasticsearch_version | stdbuf -oL tee /dev/tty) || handle_error "Failed to install Elasticsearch." "${LINENO}"
...
apt-get -qq update && apt-get -qq install kibana=$kibana_version
