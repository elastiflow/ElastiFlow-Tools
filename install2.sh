...
sudo systemctl enable opensearch && systemctl start opensearch
...
replace_text "$config_path" "#network.host: 192.168.0.1" "network.host: 0.0.0.0" "${LINENO}"
...
echo "discovery.type: single-node" | sudo tee -a "$config_path"
...
sudo systemctl enable opensearch-dashboards && systemctl start opensearch-dashboards
...
sudo systemctl enable flowcoll.service && systemctl start flowcoll.service
...
#login to opensearch-dashboards and save the cookie. note the current tenant is __user__, i guess the value for private
